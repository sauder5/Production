report 50016 "Sales Shipment-Rupp"
{
    // //SOC-SC 08-05-15
    //   Show only items
    // 
    // //RSI-KS 04-08-16
    //   Only print Seed order Complete if actually complete
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/RDLC/SalesShipmentRupp.rdlc';
        ...
        dataitem("Sales Shipment Header"; "Sales Shipment Header")
        {
            ...
            dataitem("Sales Shipment Line"; "Sales Shipment Line")
            {
                ...
                begin
                    ...
                    SetRange(Type, "Sales Shipment Line".Type::Item); //SOC-SC 08-05-15
                end;
            }
            dataitem(SalesLineBackOrders; "Sales Shipment Line")
            {
                DataItemLink = "Document No." = FIELD("No.");
                DataItemTableView = SORTING("Document No.", "Line No.") WHERE(Quantity = FILTER(0));
                ...
                trigger OnAfterGetRecord()
                begin
                    TempSalesShipmentLine := SalesLineBackOrders;
                    TempSalesShipmentLine."Line No." := SalesLineBackOrders."Line No." + 100000000;
                    TempSalesShipmentLine.Insert;
                    HighestLineNo := "Line No.";
                end;
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(SalesShptLine; "Integer")
                    {
                        ...
                        column(PrintFootertxt; sPrintFooterTxt)
                        {
                        }
                        column(FooterCaption; FooterCaptionLbl)
                        {
                        }
                        column(FooterCaption2; gsFooterTxt)
                        {
                        }
                            ...
                            with TempSalesShipmentLine do begin
                                ...
                                if "Order No." = '' then
                                    ...
                                    if OrderLine.Get(1, "Order No.", "Order Line No.") then begin
                                        ...
                                        //SOC-RB 10.1.14
                                        if BackOrderedQuantity > 0 then begin
                                            GetEstimatedReceiptDate;
                                            sPrintFooterTxt := true;
                                        end else begin
                                            EstRcptDateBackOrdered := 0D;
                                            //sPrintFooterTxt:= FALSE;
                                        end;
                                        //SOC-RB END
                                    end else begin
            ...
            begin
                ...
                //RSI-KS
                gsFooterTxt := '';
                if SalesHeader.Get(SalesHeader."Document Type"::Order, "Sales Shipment Header"."Order No.") then begin
                    SalesHeader.CalcFields(SalesHeader."Total Outstanding Qty. (base)");
                    if SalesHeader."Total Outstanding Qty. (base)" = 0 then
                        gsFooterTxt := FooterCaption2Lbl;
                end else
                    gsFooterTxt := FooterCaption2Lbl;
    ...
    var
        ...
        EstRcptDateLbl: Label 'Estd. Date';
        EstRcptDateBackOrdered: Date;
        FooterCaptionLbl: Label 'Thank you for your Order';
        FooterCaption2Lbl: Label 'Your Seed Order is now Complete';
        sPrintFooterTxt: Boolean;
        gsFooterTxt: Text;
        ...
        if OrderLine.Type = OrderLine.Type::Item then begin
            if recItem.Get(OrderLine."No.") then begin
                ...
                if BackOrderedQuantity > recItem.Inventory then begin
                    ...
                    recPurchLine.SetFilter("Future PO", '%1', false);
