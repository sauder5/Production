report 50021 "Bill of Lading - Rupp"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/RDLC/BillofLadingRupp.rdlc';
        ...
        dataitem("Sales Shipment Header"; "Sales Shipment Header")
        {
            ...
            dataitem("Sales Shipment Line"; "Sales Shipment Line")
            {
                ...
                dataitem(SalesLineComments; "Sales Comment Line")
                {
                    ...
                    DataItemTableView = SORTING("Document Type", "No.", "Document Line No.", "Line No.") WHERE("Document Type" = CONST(Shipment), "Print On Bill of Lading" = CONST(true));
            ...
            dataitem("Sales Comment Line"; "Sales Comment Line")
            {
                ...
                DataItemTableView = SORTING("Document Type", "No.", "Document Line No.", "Line No.") WHERE("Document Type" = CONST(Shipment), "Print On Bill of Lading" = CONST(true), "Document Line No." = CONST(0));
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(ShipToPhone; "Sales Shipment Header"."Ship-to Phone No. -CL-")
                    {
                    }
                    column(ShipToContact; "Sales Shipment Header"."Ship-to Contact")
                    {
                    }
                    column(PurchaseOrderNo; "Sales Shipment Header"."External Document No.")
                    {
                    }
                    column(PaymentTerms; "Sales Shipment Header"."Payment Terms Code")
                    {
                    }
                    column(ShipDate; "Sales Shipment Header"."Posting Date")
                    {
                    }
                    column(SalesPersonCode; "Sales Shipment Header"."Salesperson Code")
                    {
                    }
                    ...
                    column(ShipmentMethodDesc; "Sales Shipment Header"."Shipping Agent Code" + ' ' + "Sales Shipment Header"."E-Ship Agent Service")
                    {
                    }
                    ...
                    dataitem(SalesShptLine; "Integer")
                    {
                        ...
                        column("Order"; TempSalesShipmentLine."Order No.")
                        {
                        }
                        column(Shipped; TempSalesShipmentLine.Quantity)
                        {
                        }
                        column(GenericCode; gsGenericCode)
                        {
                        }
                        ...
                        column(EstRcptDateCaption; EstRcptDateLbl)
                        {
                        }
                        column(EstRcptDateBackOrdered; EstRcptDateBackOrdered)
                        {
                        }
                            ...
                            with TempSalesShipmentLine do begin
                                ...
                                if "Order No." = '' then
                                    ...
                                    if OrderLine.Get(1, "Order No.", "Order Line No.") then begin
                                        ...
                                        //SOC-RB 10.1.14
                                        if BackOrderedQuantity > 0 then
                                            GetEstimatedReceiptDate
                                        else
                                            EstRcptDateBackOrdered := 0D;
                                        //SOC-RB END
                                    end else begin
                                ...
                                if Type = 0 then begin
                                    ...
                                    if Type = Type::"G/L Account" then begin
                                        ...
                                        if Type = Type::Item then begin
                                            recItem.Get("No.");
                                            gsGenericCode := recItem."Generic Name Code";
                                        end;
            ...
            begin
                ...
                gsShippingMethod := '';
                gsShippingMethod := "Sales Shipment Header"."Shipping Agent Code" + ' ' + "Sales Shipment Header"."E-Ship Agent Service";
    ...
    var
        ...
        ShipmentCaptionLbl: Label 'BILL OF LADING';
        ShipmentNumberCaptionLbl: Label 'Bill of Lading:';
        ShipmentDateCaptionLbl: Label 'Shipment Date:';
        PageCaptionLbl: Label 'Page:';
        ShipViaCaptionLbl: Label 'Shipping Method';
        ...
        EstRcptDateLbl: Label 'Est. Rcpt. Date for Back Ordered';
        EstRcptDateBackOrdered: Date;
        gsShippingMethod: Text[50];
        gsGenericCode: Text[50];
    ...
    [Scope('Internal')]
    procedure GetEstimatedReceiptDate()
    var
        recPurchLine: Record "Purchase Line";
        recItem: Record Item;
    begin
        if OrderLine.Type = OrderLine.Type::Item then begin
            if recItem.Get(OrderLine."No.") then begin
                recItem.CalcFields(Inventory);
                if BackOrderedQuantity > recItem.Inventory then begin
                    recPurchLine.Reset;
                    recPurchLine.SetCurrentKey("Document Type", Type, "No.", "Variant Code", "Drop Shipment", "Location Code", "Expected Receipt Date");
                    recPurchLine.SetRange("Document Type", recPurchLine."Document Type"::Order);
                    recPurchLine.SetRange(Type, recPurchLine.Type::Item);
                    recPurchLine.SetFilter("No.", OrderLine."No.");
                    recPurchLine.SetFilter("Expected Receipt Date", '>=%1', OrderLine."Shipment Date");
                    if recPurchLine.FindSet then
                        EstRcptDateBackOrdered := recPurchLine."Expected Receipt Date"
                    else
                        EstRcptDateBackOrdered := 0D;
                end else
                    EstRcptDateBackOrdered := Today;
            end;
        end else
            EstRcptDateBackOrdered := 0D;
    end;
